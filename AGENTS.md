...
[HARD] D-20260908-06 已授权每日北京时间 07:15 触发本项目的检索、核验、生成、提交推送及 Pages 发布流程。按 automation-rules A-02/A-03/A-10 执行，质量失败保留草稿，禁止自动微信发送。先前冻结记录不再阻断授权的日常流程；定时器启用不代表采集覆盖和端到端实测全部完成。
...
当前内容修订以 D-20260908-02 为准：允许本地修订两期和历史集合并预览；会议临时阻断及四条首期方案已废弃，不能沿用。日常发布现按 D-20260908-06 授权执行。
